...
UPDATE qgep.od_wastewater_networkelement SET obj_id=qgep.generate_oid('od_reach_point')
WHERE qgep.od_wastewater_networkelement.old_obj_id IN (SELECT obj_id FROM abwasser.siedlungsentwaesserung__Haltungspunkt);

-- 4. einsetzen der daten von siedlungsentwaesserung in qgep.
INSERT INTO qgep.od_reach_point
(obj_id, old_obj_id, elevation_accuracy, identifier, level, outlet_shape, position_of_connection, remark, situation_geometry, last_modification, Dataowner) 
...
MD_DATENHERR
FROM abwasser.siedlungsentwaesserung__Haltungspunkt a_haltepunkt
LEFT JOIN qgep.od_wastewater_networkelement q_wastewater_networkelement
ON a_haltepunkt.obj_id = q_wastewater_networkelement.old_obj_id;
